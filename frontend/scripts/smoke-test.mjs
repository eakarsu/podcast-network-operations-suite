const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5800';
...
const adminCookie = await login('admin@podcast-network-operations.local', 'admin123');
const managerCookie = await login('manager@podcast-network-operations.local', 'manager123');
const analystCookie = await login('analyst@podcast-network-operations.local', 'analyst123');
